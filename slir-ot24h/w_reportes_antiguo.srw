HA$PBExportHeader$w_reportes_antiguo.srw
forward
global type w_reportes_antiguo from window
end type
type st_titulos from statictext within w_reportes_antiguo
end type
type mle_titulos from multilineedit within w_reportes_antiguo
end type
type st_subtitulos from statictext within w_reportes_antiguo
end type
type mle_subtitulos from multilineedit within w_reportes_antiguo
end type
type p_1 from picture within w_reportes_antiguo
end type
type st_titulo from statictext within w_reportes_antiguo
end type
type dw_funciones from u_pr_funcion within w_reportes_antiguo
end type
type st_3 from statictext within w_reportes_antiguo
end type
type st_2 from statictext within w_reportes_antiguo
end type
type st_1 from statictext within w_reportes_antiguo
end type
type dw_lista_orden from u_lista_orden within w_reportes_antiguo
end type
type sle_1 from singlelineedit within w_reportes_antiguo
end type
type dw_lista_items from u_lista_reporte within w_reportes_antiguo
end type
type uo_menu from u_pr_menu within w_reportes_antiguo
end type
type st_material from statictext within w_reportes_antiguo
end type
type st_causa from statictext within w_reportes_antiguo
end type
type mle_1 from multilineedit within w_reportes_antiguo
end type
type dw_1 from datawindow within w_reportes_antiguo
end type
type dw_ambito from u_cen_2001_pr_form_centro_cmd_mesa within w_reportes_antiguo
end type
type dw_filtro_reporte from u_gen_7001_pr_filtro_reporte_inc within w_reportes_antiguo
end type
type st_instalacion from statictext within w_reportes_antiguo
end type
end forward
...
string title = "OPEN SGI - Objeto de Creaci$$HEX1$$f300$$ENDHEX$$n de Informes Din$$HEX1$$e100$$ENDHEX$$micos."
...
global w_reportes_antiguo w_reportes_antiguo
...
For li_count = 1 to dw_lista_items.rowcount()
	if dw_lista_items.isselected(li_count) then
		...
		if li_filas_selec=1 then
			if pos(ls_campo,"PL_")>0 then // EL CAMPO ES UN PLSQL  ACO
				ls_select= " SELECT " +  ls_campo
			else
				ls_select= " SELECT " + ls_tabla + "." + ls_campo
			end if
	    	...
	    	if pos(ls_campo,"PL_")>0 then // EL CAMPO ES UN PLSQL  ACO
				 ls_select = ls_select + " , " + ls_campo
			else
				ls_select = ls_select + " , " + ls_tabla + "." + ls_campo
			end if
			if not lb_funcion_aplicada and ib_funcion then
			...
			choose case li_cmd
					case 0
						if li_cant_joins >0 then
							...
							+ string(li_centro)+ " and " + is_tabla_consulta + ".nro_cmd >0 "
						else
