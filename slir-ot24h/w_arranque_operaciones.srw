HA$PBExportHeader$w_arranque_operaciones.srw
forward
global type w_arranque_operaciones from window
end type
type cb_2 from commandbutton within w_arranque_operaciones
end type
type st_8 from statictext within w_arranque_operaciones
end type
type st_7 from statictext within w_arranque_operaciones
end type
type st_6 from statictext within w_arranque_operaciones
end type
type st_3 from statictext within w_arranque_operaciones
end type
type p_1 from picture within w_arranque_operaciones
end type
type p_uf from picture within w_arranque_operaciones
end type
type st_2 from statictext within w_arranque_operaciones
end type
type cb_3 from u_cb within w_arranque_operaciones
end type
type cb_1 from u_cb within w_arranque_operaciones
end type
type st_1 from statictext within w_arranque_operaciones
end type
type oval_1 from oval within w_arranque_operaciones
end type
type st_5 from statictext within w_arranque_operaciones
end type
type st_4 from statictext within w_arranque_operaciones
end type
type d_usuario from u_gen_2001_pr_ingreso_usuario within w_arranque_operaciones
end type
type sle_nuevaclave from singlelineedit within w_arranque_operaciones
end type
end forward
...
global w_arranque_operaciones w_arranque_operaciones
...
//AHM (28/01/2010) Incidencia 0/100163598
string	ls_nulo						//Valor nulo 
	...
	IF ps_fechaRenovacion < ls_fechaProximaRen THEN
		...
		IF ii_numeroIntentos > 1 THEN
			IF ii_numeroPasadas = 0 THEN			//Comprueba que es la primera vez que entramos
				...
				cb_3.visible = TRUE
				//AHM (28/01/2010) Incidencia 0/100163598
				IF ClassName() <> "w_arranque_operaciones_ext" THEN
					st_7.visible = TRUE
					st_8.visible = TRUE
				...
				ELSE
					d_usuario.enabled = TRUE
					d_usuario.object.ps_new_clave.protect = 0 
					d_usuario.object.ps_new_clave.TabSequence = 40 
					d_usuario.object.ps_new_clave.Background.color = gs_blanco
					d_usuario.setColumn("ps_new_clave")
				END IF
				setredraw(TRUE)
			...
			ELSEIF ii_numeroPasadas = 1 THEN //Se introduce la clave la primera vez
				...
				ELSE
					is_nuevaClave1 =  d_usuario.getItemString(1, 'ps_new_clave')
				END IF
				...
				ELSE
					...
					d_usuario.object.ps_new_clave.TabSequence = 40 
					d_usuario.object.ps_new_clave.Background.color = gs_blanco
					d_usuario.setItem(1, "ps_new_clave", ls_nulo)
					d_usuario.setColumn("ps_new_clave")
				END IF
				setredraw(TRUE)
				CLOSE(w_barra_standart)
			...
			ELSEIF ii_numeroPasadas	= 2 THEN //Se introduce la clave la segunda vez
				...
				ELSE
					is_nuevaClave2 =  d_usuario.getItemString(1, 'ps_new_clave')
				END IF
...
//	This.SetRedraw(True)
//	
//	//AHM 19/04/2011 ASUR 1031287
//	IF ProfileString("opensgi.ini", "OPEN-SGI", "usuario_red", "") = "NO" THEN
...
on w_arranque_operaciones.create
...
on w_arranque_operaciones.destroy
destroy(this.cb_2)
destroy(this.st_8)
destroy(this.st_7)
destroy(this.st_6)
...
event timer;//AHM (08/06/2011) Cambio de logo
...
IF  ProfileString("opensgi.ini","OPEN-SGI","gasnaturalfenosa","N") = "SI"  THEN
	p_uf.picturename="gasNaturalFenosa.BMP"
ELSE
		choose case i_vez
		case 1 
			 ib_subiendo=true
...
d_usuario.object.ps_usuario[1] = 'RICCI'
d_usuario.accepttext()
...
st_7.text='BUFALO_DESSF9'

SetPointer(HourGlass!)
timer(0)
string ls_usu,ls_nombre,ls_clave, ls_clave_introducida, ls_cifrada, ls_usuario_ini
string ls_fase
...
ls_usuario_ini=ProfileString("opensgi.ini","OPEN-SGI","usuario_sgi","SINUSUARIO")
...
//AHM(19/04/2011) ASUR 1031287
SetProfileString("opensgi.ini","OPEN-SGI","usuario_sgi",ls_usuario)
...
//NCA-DDAG-1530.29/01/2015.INICIO.Buscamos si el perfil tiene acceso o no a la opcion INTERFAZ_OOP

	SELECT "GI_OPCIONES_PERFIL"."ACCESO"
	  INTO : ll_acceso
     FROM "GI_OPCIONES_PERFIL", GI_OPCIONES  
    WHERE "GI_OPCIONES_PERFIL"."PERFIL" = :li_per
	   AND GI_OPCIONES.OPCION = GI_OPCIONES_PERFIL.OPCION
      AND "GI_OPCIONES"."OPCION" = 'INTERFAZ_OOP';
		

		//Si no tiene acceso asignamos en falso la variable gb_operaciones para que no se conecte.			
		If ll_acceso = 441 Then 		
		   gb_operaciones = FALSE
		End If 	
