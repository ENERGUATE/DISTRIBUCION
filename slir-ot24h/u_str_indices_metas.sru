HA$PBExportHeader$u_str_indices_metas.sru
forward
global type u_str_indices_metas from userobject
end type
type cb_1 from commandbutton within u_str_indices_metas
end type
type cb_recuperar from commandbutton within u_str_indices_metas
end type
type dw_filtro from datawindow within u_str_indices_metas
end type
type cb_calcular from commandbutton within u_str_indices_metas
end type
type cb_cierremensual from commandbutton within u_str_indices_metas
end type
type cb_filtrar from commandbutton within u_str_indices_metas
end type
type dw_activos from datawindow within u_str_indices_metas
end type
type dw_periodo from datawindow within u_str_indices_metas
end type
type gb_1 from groupbox within u_str_indices_metas
end type
type gb_2 from groupbox within u_str_indices_metas
end type
end forward
...
global u_str_indices_metas u_str_indices_metas
...
n_calculos_indicadores inv_calculos_str
end variables
...
public subroutine uf_set_fecha (long pl_anio, long pl_mes)
public subroutine uf_generar_informe ()
end prototypes
...
public subroutine uf_filtrar_activos ();
boolean lb_instalacion = true, lb_tipo_grupo = true 
...
//dw_activos.Filter()
//
//if isnull(dw_filtro.object.tipo_grupo[1]) or dw_filtro.object.tipo_grupo[1] = 999 then 
//	lb_tipo_grupo = false
//else
//	ll_tipo_grupo = dw_filtro.object.tipo_grupo[1] 	
//	ls_filter = "TIPO_GRUPO = " + string(ll_tipo_grupo)	
//end if
//
//
//if isnull(dw_filtro.object.nombre_grupo[1]) or dw_filtro.object.nombre_grupo[1]= '' then 	
//	lb_instalacion = false
//else
//
//	if lb_tipo_grupo then ls_filter += ' AND '
//
//	ls_filter += "DESCRIPCION LIKE '" + dw_filtro.object.nombre_grupo[1] + "%'"
//	
//end if
//		
...
dw_filtro.object.tipo_grupo[1] = 999
dw_filtro.object.nombre_grupo[1] = ''
...
public subroutine uf_set_fecha (long pl_anio, long pl_mes);
dw_periodo.object.anio[1] = pl_anio
dw_periodo.object.mes[1] = pl_mes

end subroutine

public subroutine uf_generar_informe ();

dw_activos.SaveAs() //"c:", Excel!, FALSE)
end subroutine

on u_str_indices_metas.create
...
on u_str_indices_metas.destroy
destroy(this.cb_1)
...
inv_calculos_str = CREATE n_calculos_indicadores
...
event clicked;uf_generar_informe()
end event
...
integer taborder = 30
string title = "none"
string dataobject = "d_indicadores_filtrar_grupo_activos"
...
this.GetChild('tipo_grupo', ldddw_tipo_activo)
...
date ldate_fecha_hasta
string ls_fecha_auxiliar, ls_dwis_auxiliar
decimal{4} ldec_indice_indisponibilidad, ldec_mida, ldec_auxiliar, ldec_imr
long ll_mhaia, ll_sce, ll_cpsm, ll_enr
...
//		ls_dwis_auxiliar = uf_get_itemstatus(ldwis_auxiliar)
...
//						
//		inv_calculos_str.uf_calcular_indice_disponibilidad(ll_activo, datetime(ldate_fecha_hasta), ldec_indice_indisponibilidad, ll_sce, ll_cpsm, ll_enr)
//		
//		dw_activos.object.indice_disponibilidad[ll_cont] = ldec_indice_indisponibilidad		
//		dw_activos.object.cnp[ll_cont] = ll_sce + ll_cpsm		
...
//		
//		ldec_imr = inv_calculos_str.uf_calcular_ingresomensualregulado(ll_activo, ldate_fecha_hasta)
//		
...
integer taborder = 20
string title = "none"
string dataobject = "d_str_indices_metas"
...
this.GetChild('codigo_grupo', ldddw_codigo)

ldddw_codigo.SetTransObject(SQLCA)
ldddw_codigo.Retrieve()
...
event doubleclicked;
IF row > 0 then 
 openwithparm(w_str_detalle_indicadores_response, dw_activos)
end if
end event
